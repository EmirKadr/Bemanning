...
7. Vid lyckad uppladdning visas hur manga filer som sparades. Vid fel visas ett kort felmeddelande pa sidan.
...
| Ladda upp | `meta-upload.html` | Alla med lank | Skickar valda filer som multipart till backend och visar progress via `XMLHttpRequest.upload` | `POST /api/meta/uploads` | Nekas om inga filer skickas, om filtypen inte ar bild/video eller om maxstorlek passeras. |
...
- Tabellen sparar `original_filename`, `stored_filename`, `content_type`, `media_type`, `size_bytes`, binar `data`, `status`, `analysis`, `source` och `created_at`.
