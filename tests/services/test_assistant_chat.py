...
def test_assistant_user_context_uses_configured_view_access():
    ...
    context = assistant.build_user_context(
        user,
        role_access={"warehouse_clerk": {"allocationTrace": "none", "allocationSplit": "edit"}},
        page_path="/harleda.html",
    ...
    assert "Härleda (`allocationTrace`)" in context
    assert "Aktuell sidas vybehörighet: Härleda (`allocationTrace`) = `none`" in context
