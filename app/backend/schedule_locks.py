...
from .user_access import STAFFING_MANAGER_ROLE, can_admin, is_super_user, user_roles
...
def user_can_bypass_schedule_cell_lock(user: User) -> bool:
    return can_admin(user) or is_super_user(user) or STAFFING_MANAGER_ROLE in user_roles(user)
