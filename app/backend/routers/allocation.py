...
@router.get("/flows")
def list_flows(_user: User = Depends(require_allocation_tools_user)) -> dict:
    _engine_module, flows_module = bridge.require_available()
    return {"flows": flows_module.public_registry()}


@router.get("/pool")
def list_pool(_user: User = Depends(require_allocation_tools_user)) -> dict:
    _engine_module, flows_module = bridge.require_available()
